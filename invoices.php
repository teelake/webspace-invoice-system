...
<div class="section-header">
    <h2>All Invoices</h2>
    <a href="invoice-edit" class="btn btn-primary">+ New Invoice</a>
</div>
...
async function loadInvoices() {
    ...
    tbody.innerHTML = list.length ? list.map(inv => `
        <tr>
            ...
            <td>${inv.due_date}</td>
            <td>
                <a href="invoice-view?id=${inv.id}" class="btn btn-sm btn-secondary">View</a>
                <a href="invoice-edit?id=${inv.id}" class="btn btn-sm">Edit</a>
            </td>
        </tr>
    `).join('') : '<tr><td colspan="7"><div class="empty-state"><div class="empty-state-icon">📄</div><div class="empty-state-title">No invoices yet</div><div class="empty-state-text">Create your first invoice to get started</div><a href="invoice-edit" class="btn btn-primary" style="margin-top:1rem">+ New Invoice</a></div></td></tr>';
